...
const tagClass = (tag?: string) => {
  ...
  if (t.includes("react")) return "bg-sky-800/20 text-sky-300 border-sky-300/10";
  if (t.includes("typescript") || t.includes("ts")) return "bg-indigo-800/20 text-indigo-300 border-indigo-300/10";
  if (t.includes("tailwind")) return "bg-teal-800/20 text-teal-300 border-teal-300/10";
  if (t.includes("node") || t.includes("node.js")) return "bg-emerald-800/20 text-emerald-300 border-emerald-300/10";
  if (t.includes("python")) return "bg-amber-800/20 text-amber-300 border-amber-300/10";
  if (t.includes("cli") || t.includes("ink")) return "bg-violet-800/20 text-violet-300 border-violet-300/10";
  if (t.includes("api") || t.includes("rest")) return "bg-rose-800/20 text-rose-300 border-rose-300/10";
  if (t.includes("ai")) return "bg-fuchsia-800/20 text-fuchsia-300 border-fuchsia-300/10";
  if (t.includes("bun")) return "bg-orange-800/20 text-orange-300 border-orange-300/10";
    ...
    <article className="group relative rounded-2xl overflow-hidden border border-white/6 bg-linear-to-tr from-white/2 to-white/4 backdrop-blur-sm shadow-sm hover:shadow-lg transition-transform transform hover:-translate-y-1 cursor-pointer">
      ...
      <div className="p-4 md:p-5 flex flex-col gap-3">
        ...
        <div className="mt-3 flex items-center justify-between text-xs text-gray-400">
          <span className="opacity-90">Made by <strong className="text-gray-200">vathsavv56</strong></span>
          <div className="flex items-center gap-3">
            ...
            {live && <a href={live} target="_blank" rel="noopener noreferrer" className="hover:underline">Live</a>}
          </div>
        </div>
      </div>
    </article>
