...
const GithubHeatmap = ({ username }: { username: string }) => {
  ...
  return (
    <div className="w-full p-3 sm:p-4 border rounded-xl bg-[#0d1117] border-zinc-800 overflow-hidden">
      ...
      */}
      <div className="grid grid-rows-7 grid-flow-col gap-px sm:gap-0.5 w-max mx-auto">
        {loading
          ? // Loading skeletons
            Array.from({ length: 364 }).map((_, i) => (
              <div
                key={`skeleton-${i}`}
                className="w-1 h-1 sm:w-1.5 sm:h-1.5 md:w-2 md:h-2 lg:w-2.5 lg:h-2.5 rounded-full bg-zinc-800/50 animate-pulse"
              />
            ))
          : // Actual contribution data
            data.map((day) => (
              <div
                key={day.date}
                className="w-1 h-1 sm:w-1.5 sm:h-1.5 md:w-2 md:h-2 lg:w-2.5 lg:h-2.5 rounded-full cursor-pointer hover:scale-125 hover:ring-1 hover:ring-zinc-400 hover:z-10 transition-all duration-200"
